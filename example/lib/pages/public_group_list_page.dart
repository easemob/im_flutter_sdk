...
class _PublicGroupListPageState extends State<PublicGroupListPage>{
  ...
  void _getPublicGroups(){
    EMClient.getInstance().groupManager().getPublicGroupsFromServer(pageSize: 20, cursor: '',
    onSuccess: (result){
  ...
  void _onTap(EMGroupInfo groupInfo){
    EMClient.getInstance().groupManager().getGroupFromServer(groupId:groupInfo.getGroupId(),
        onSuccess: (group){
          if(group.isMemberOnly()) {
            EMClient.getInstance().groupManager().applyJoinToGroup(groupId: group.getGroupId(), reason: '',
              onSuccess: (){
            ...
            EMClient.getInstance().groupManager().joinGroup(groupId: group.getGroupId(),
            onSuccess: (){
