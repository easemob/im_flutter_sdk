...
class _PublicGroupListPageState extends State<PublicGroupListPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(DemoLocalizations.of(context).publicGroups,
            style: TextStyle(
                color: ThemeUtils.isDark(context)
                    ? EMColor.darkText
                    : EMColor.text)),
  ...
  void _onTap(EMGroupInfo groupInfo) {
    EMClient.getInstance()
        .groupManager()
        .getGroupFromServer(groupInfo.getGroupId(), onSuccess: (group) {
      if (group.isMemberOnly()) {
        EMClient.getInstance()
            .groupManager()
            .applyJoinToGroup(group.getGroupId(), '', onSuccess: () {
