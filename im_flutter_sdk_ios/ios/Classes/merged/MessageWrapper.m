...
- (void)getReaction:(NSDictionary *)param
    ...
    EMMessageReaction *msgReaction = nil;
    for (EMMessageReaction *r in msg.reactionList) {
        if ([r.reaction isEqualToString:reaction]) { msgReaction = r; break; }
    }
...
- (void)getGroupAckCount:(NSDictionary *)param
    ...
    [self wrapperCallBack:result
                  channelName:aChannelName
                        error:nil
                       object:@(msg.groupReadReceiptCount)];
