...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([ChatAddContact isEqualToString:call.method]) {
        ...
        [self fetchContacts:call.arguments
                            channelName:call.method
                                 result:result];
    } else if ([ChatSaveBlackList isEqualToString:call.method]) {
        [self saveBlackList:call.arguments channelName:call.method result:result];
    } else {
...
- (void)getAllContactsFromServer:(NSDictionary *)param channelName:(NSString *)aChannelName result:(FlutterResult)result {
    __weak typeof(self)weakSelf = self;
    // iOS 5.0 移除服务端拉取联系人，改用本地 getContacts
    NSArray<NSString *> *list = [EMClient.sharedClient.contactManager getContacts];
    [weakSelf wrapperCallBack:result channelName:aChannelName error:nil object:list];
}
...
- (void)fetchAllContacts:(NSDictionary *)param channelName:(NSString *)aChannelName result:(FlutterResult)result {
    __weak typeof(self)weakSelf = self;
    // iOS 5.0 移除服务端拉取联系人，改用本地 getAllContacts
    NSArray<EMContact *> *aList = [EMClient.sharedClient.contactManager getAllContacts];
    NSMutableArray *contactList = [NSMutableArray array];
    for (EMContact *contact in aList) {
        [contactList addObject:[contact toJson]];
    }
    [weakSelf wrapperCallBack:result channelName:aChannelName error:nil object:contactList];
}


- (void)fetchContacts:(NSDictionary *)param channelName:(NSString *)aChannelName result:(FlutterResult)result {
    __weak typeof(self)weakSelf = self;
    // iOS 5.0 移除 getContactsFromServerWithCursor 分页拉取；改用本地 getContacts
    NSArray<NSString *> *list = [EMClient.sharedClient.contactManager getContacts];
    [weakSelf wrapperCallBack:result channelName:aChannelName error:nil object:list];
}
...
- (void)saveBlackList:(NSDictionary *)param channelName:(NSString *)aChannelName result:(FlutterResult)result {
    __weak typeof(self) weakSelf = self;
