...
- (void)loginWithAgoraToken:(NSDictionary *)param channelName:(NSString *)aChannelName result:(FlutterResult)result {
    __weak typeof(self) weakSelf = self;
    // 5.0 移除 agoraToken 登录，统一 loginWithUsername:token:
    NSString *username = param[@"userId"];
...
{
    __weak typeof(self)weakSelf = self;
    // 5.0 移除全局已读回执开关（改为消息级 isNeedReadReceipt）
    [weakSelf wrapperCallBack:result
                  channelName:aChannelName
                        error:nil
                       object:nil];
...
- (void)getRTCTokenInfoWithChannelName:(NSDictionary *)param channelName:(NSString *)aChannelName result:(FlutterResult)result {
    __weak typeof(self) weakSelf = self;
    NSString *channel = param[@"channelName"];
    [EMClient.sharedClient getRTCTokenWithChannel:channel
                                       completion:^(NSUInteger rtcUId, NSString * _Nullable aToken, NSInteger expiredTs, EMError * _Nullable aError) {
        NSDictionary *obj = @{@"rtcToken": aToken ?: @"", @"rtcUid": @(rtcUId), @"expiredTs": @(expiredTs)};
        [weakSelf wrapperCallBack:result channelName:aChannelName error:aError object:obj];
    }];
}
