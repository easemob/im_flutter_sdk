...
class ChatUserInfoManager {
  ...
  final Map<String, ChatUserInfoEventHandler> _eventHandlesMap = {};
  ...
  Future<void> _onUserInfoChanged(Map event) async {
    ...
    for (var element in _eventHandlesMap.values) {
      switch (type) {
        case ChatUserInfoChangeEvent.ON_SELF_USER_INFO_UPDATE:
          ChatUserInfo userInfo = ChatUserInfo.fromJson(event['userInfo']);
          element.onSelfUserInfoUpdate?.call(userInfo);
          break;
        case ChatUserInfoChangeEvent.ON_USER_INFO_UPDATE:
          List<ChatUserInfo> userInfos = [];
          event['userInfos']?.forEach((obj) {
            userInfos.add(ChatUserInfo.fromJson(obj));
          });
  ...
  ///
  /// Param [handler] The handler for user info events. See [ChatUserInfoEventHandler].
  /// ~end
  ///
  /// ~chinese
  ...
  ///
  /// Param [handler] 事件的句柄。请参见 [ChatUserInfoEventHandler]。
  /// ~end
  void addEventHandler(
    String identifier,
    ChatUserInfoEventHandler handler,
  ) {
  ...
  /// **Return** 事件的句柄。
  /// ~end
  ChatUserInfoEventHandler? getEventHandler(String identifier) {
  ...
  ///
  /// **Return** The list of subscribed users. See [ChatUserInfo].
  ///
  /// **Throws** A description of the exception. See [ChatError].
  /// ~end
  ///
