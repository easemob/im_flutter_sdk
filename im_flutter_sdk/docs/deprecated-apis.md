...
- `ChatOptions` 不再能配置厂商推送的 appId/appKey/证书名，也不再序列化 `pushConfig` 字段（native 侧不可达的读取分支已随之删除：Android `EMHelper.java`、iOS `OptionsHelper.m`）；承载这些配置的 `ChatPushConfig` 类连同 `lib/src/internal/chat_push_config.dart`、`inner_headers.dart` 的导出与 `em_compat.dart` 的 `EMPushConfig` typedef 一并删除。
