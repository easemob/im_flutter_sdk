...
- [x] 新增 `native-auto-test/tests/group/test_group_capacity.py` 的无设备单元测试：默认容量、配置为 3100、拒绝非正整数。
...
- [ ] 以 `--group-create-max-count=3100` 对一个常规 Group 创建 case 做 discovery，确认请求和返回容量均为 3100；随后运行严格回归。未执行：该步骤会写入共享测试环境，需当前明确授权。
- [x] 更新 `native-auto-test/docs/agents/group/CASES_RECORD.zh.md`，记录两个运行命令、边界值豁免和静态验证结果。
- [x] 回填本文件所有任务状态；设计已同步为 pytest 配置/清理钩子实现。

## 2026-09-15 用例精简说明

上述容量单元测试记录为历史验证。用户已要求删除对应5个节点及测试文件，不再执行其历史命令，也未迁移到tools。容量配置功能保留；本次删除与验证状态统一记录于 `../release-test-automation/tasks.md` 的“用例精简”章节。3100真实群场景仍未执行。
