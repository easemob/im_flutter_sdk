...
User Story：作为回归维护者，我希望已有第三方好友不会导致 A/B 离线关系用例误报。
- WHEN A/B 关系清理完成 THEN 用例 SHALL 读取双方独立好友基线并验证对端已不存在。
- WHEN 业务操作完成 THEN 用例 SHALL 严格核对完整好友列表等于基线加上预期对端，忽略顺序但不忽略新增、丢失或重复好友。
