...
/**
 * Web wrapper entry point.  Web 5.0 uses the official IIFE global; the legacy
 * 4.23 path remains available for the old mixed-platform scenario.
 */
export function createWebSdkWrapper({
  ...
  return Number.parseInt(String(sdkVersion).split(".")[0], 10) >= 5
    ? createWeb5SdkWrapper({ appKey, emit, onStatus })
    : createLegacyWebSdkWrapper({ appKey, emit, onStatus });
}

function createLegacyWebSdkWrapper({ appKey, emit, onStatus }) {
  let currentUser = "";
  const messages = new Map();
  const connection = new WebIM.connection({ appKey, delivery: true, isFixedDeviceId: true });
  ...
  return { invoke: invokeLegacy };

  async function invokeLegacy(manager, cmd, info = {}) {
    if (manager === "Client") return invokeLegacyClient(cmd, info);
    if (manager === "ChatManager") return invokeLegacyChat(cmd, info);
    throw new Error(`Web 4.23 wrapper does not support ${manager}.${cmd}`);
  }

  async function invokeLegacyClient(cmd, info) {
    ...
    if (cmd === "login") {
      ...
      const result = await connection.open({
        user: currentUser,
        ...(info.isPassword === false ? { accessToken: info.pwdOrToken } : { pwd: info.pwdOrToken }),
      });
    ...
    throw new Error(`Web 4.23 wrapper does not support Client.${cmd}`);
  }

  async function invokeLegacyChat(cmd, info) {
    if (cmd === "sendMessage") {
      const body = info.body || {};
      if (body.type !== 0) throw new Error("Web 4.23 wrapper only supports text messages");
      const raw = WebIM.message.create({ chatType: chatType(info.chatType), type: "txt", to: info.to, msg: body.content });
      ...
      const canonical = canonicalMessage({ ...raw, id: realId, from: currentUser, to: info.to, msg: body.content }, { direction: 0, currentUser });
    ...
    if (cmd === "addReaction") {
      await connection.addReaction({ messageId: String(info.msgId), reaction: String(info.reaction) });
      return null;
    }
    throw new Error(`Web 4.23 wrapper does not support ChatManager.${cmd}`);
  }
}
