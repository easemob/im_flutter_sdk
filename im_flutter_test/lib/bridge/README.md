...
默认连接：`ws://127.0.0.1:4000/iov/websocket/dual?topic=adc`。可通过 `start(topic: ...)` 自定义 topic，或通过 `start(url: ...)` 指定完整 URL。
...
// 指定完整 URL
await IMWebSocketBridge.instance.start(
  url: 'ws://127.0.0.1:4000/iov/websocket/dual?topic=adc',
);
```
...
BASE = "ws://127.0.0.1:4000/iov/websocket/dual"
