...
- ChatThread 属于群组场景；实现文件已统一迁移到 `tests/group/`，本文件保留 API 维度台账。
...
正常 cases
1. `tests/group/test_group_chat_thread_user_removed.py::test_chat_thread_remove_member_updates_member_list`
   A 建群并基于群父消息创建子区，B 加入后 A 将 B 移出，查询子区成员确认 B 已移除，最后销毁子区；当前 Android 未稳定派发 `onUserKickOutOfChatThread`，因此不伪造该回调断言。
...
正常 cases
3. `tests/group/test_group_chat_thread_remaining_api_coverage.py::test_chat_thread_fetch_detail_and_lists`
...
正常 cases
5. `tests/group/test_group_chat_thread_remaining_api_coverage.py::test_chat_thread_fetch_members_and_latest_message`
...
正常 cases
7. `tests/group/test_group_chat_thread_remaining_api_coverage.py::test_chat_thread_update_name_and_leave`
...
8. `tests/group/test_group_chat_thread_remaining_api_coverage.py::test_chat_thread_destroy_event_received_by_group_member`
