...
49. `tests/contact/test_contact_offline_friendship.py::test_contact_offline_recipient_receives_delete_after_relogin`
    好友关系建立后 B 先离线，A 删除 B；严格断言 A 的删除响应和即时 `onContactDeleted`、B 重登后的离线 `onContactDeleted`，以及双方服务端好友列表均为空。
50. `tests/contact/test_contact_offline_friendship.py::test_contact_offline_requester_receives_peer_delete_after_relogin`
    好友关系建立后 A 先离线，B 删除 A；严格断言 B 的删除响应和即时 `onContactDeleted`、A 重登后的离线 `onContactDeleted`，以及双方服务端好友列表均为空。
