...
def test_chatroom_is_member_in_white_list_and_mute_list_success(device_a, assert_api, user_a):
    ...
    try:
        with _allure_step("查询聊天室白名单成员状态并验证返回布尔值"):
            resp_white = device_a.call(
                "ChatRoomManager", Cmd.isMemberInChatRoomWhiteListFromServer.value, info={"roomId": room_id}
            )
            assert_api.assert_response_matches(
                resp_white,
                expected={"manager": "ChatRoomManager", "cmd": Cmd.isMemberInChatRoomWhiteListFromServer.value, "device": "deviceA"},
        ...
        with _allure_step("查询聊天室禁言名单成员状态并验证返回布尔值"):
            resp_mute = device_a.call(
                "ChatRoomManager", Cmd.isMemberInChatRoomMuteList.value, info={"roomId": room_id}
            )
            assert_api.assert_response_matches(
                resp_mute,
                expected={"manager": "ChatRoomManager", "cmd": Cmd.isMemberInChatRoomMuteList.value, "device": "deviceA"},
                ignore_keys={"sequence", "result"},
            )
...
def test_chatroom_is_member_in_white_list_and_mute_list_nonexistent_room(device_a, assert_api):
    ...
    with _allure_step("查询不存在聊天室的白名单和禁言名单状态并验证错误"):
...
def test_chatroom_member_white_list_check_reflects_server_state(device_a, device_b, assert_api, user_a, user_b):
    ...
    try:
        with _allure_step("B 加入聊天室并验证初始不在白名单"):
        ...
        with _allure_step("A 添加 B 到白名单并验证 B 可查询到白名单状态"):
            add_resp = device_a.call(
                "ChatRoomManager", Cmd.addMembersToChatRoomWhiteList.value,
                info={"roomId": room_id, "members": [user_b]},
            )
            assert_api.assert_response_matches(
                add_resp,
                expected={"manager": "ChatRoomManager", "cmd": Cmd.addMembersToChatRoomWhiteList.value, "device": "deviceA"},
                ignore_keys={"sequence", "result"},
            )
        ...
        with _allure_step("A 移除 B 出白名单并验证状态恢复"):
            remove_resp = device_a.call(
                "ChatRoomManager", Cmd.removeMembersFromChatRoomWhiteList.value,
                info={"roomId": room_id, "members": [user_b]},
            )
            assert_api.assert_response_matches(
                remove_resp,
                expected={"manager": "ChatRoomManager", "cmd": Cmd.removeMembersFromChatRoomWhiteList.value, "device": "deviceA"},
                ignore_keys={"sequence", "result"},
            )
...
def test_chatroom_member_mute_list_check_reflects_server_state(device_a, device_b, assert_api, user_a, user_b):
    ...
    try:
        with _allure_step("B 加入聊天室并验证初始不在禁言列表"):
            join_resp = device_b.call("ChatRoomManager", Cmd.joinChatRoom.value, info={"roomId": room_id})
            assert_join_chatroom_response(assert_api, join_resp, device="deviceB", room_id=room_id)
            before_resp = device_b.call(
                "ChatRoomManager", Cmd.isMemberInChatRoomMuteList.value, info={"roomId": room_id}
        ...
        with _allure_step("A 禁言 B 并验证 B 查询到禁言状态"):
            mute_resp = device_a.call(
                "ChatRoomManager", Cmd.muteChatRoomMembers.value,
                info={"roomId": room_id, "muteMembers": [user_b], "duration": 60000},
            )
            assert_api.assert_response_matches(
                mute_resp,
                expected={"manager": "ChatRoomManager", "cmd": Cmd.muteChatRoomMembers.value, "device": "deviceA"},
                ignore_keys={"sequence", "result"},
            )
        ...
        with _allure_step("A 解除 B 禁言并验证状态恢复"):
            unmute_resp = device_a.call(
                "ChatRoomManager", Cmd.unMuteChatRoomMembers.value,
                info={"roomId": room_id, "unMuteMembers": [user_b]},
            )
            assert_api.assert_response_matches(
                unmute_resp,
                expected={"manager": "ChatRoomManager", "cmd": Cmd.unMuteChatRoomMembers.value, "device": "deviceA"},
                ignore_keys={"sequence", "result"},
            )
