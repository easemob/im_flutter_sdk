...
def test_client_sync_has_single_10_second_budget(clock):
...
def test_group_collection_respects_exact_idle_grace(clock):
...
def test_offline_group_setup_waits_for_readiness_not_fixed_sleep(clock, monkeypatch, module, helper, drain_budget):
    ...
    assert clock.now == pytest.approx(drain_budget + 0.05)


def test_chatroom_ext_observer_returns_without_half_second_collection(clock, monkeypatch):
