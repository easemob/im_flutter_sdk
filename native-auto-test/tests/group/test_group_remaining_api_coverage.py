...
def test_group_clear_all_groups_from_local_success(device_a, assert_api, user_a):
    ...
    with _allure_step("A 清理本地群数据"):
        resp = device_a.call("GroupManager", Cmd.clearAllGroupsFromDB.value)
    with _allure_step("验证清理本地群数据返回的响应 result 与关键字段"):
        assert_api.assert_response_matches(
            resp,
            expected={
                "manager": "GroupManager",
...
def test_group_fetch_members_info_invalid_limit(device_a, assert_api, user_a):
    ...
    group_id = ""
    try:
        with _allure_step("A 查询用户资料"):
            resp_user_info = device_a.call(
                "UserInfoManager",
                Cmd.fetchUserInfoById.value,
                info={"userIds": [user_a]},
            )
        with _allure_step("验证查询用户资料返回的响应 result 与关键字段"):
            assert_api.assert_response_matches(
                resp_user_info,
                expected={
        ...
        with _allure_step("验证查询群成员信息返回的响应 result 与关键字段"):
            assert_api.assert_response_matches(
                resp,
                expected={
                    "manager": "GroupManager",
