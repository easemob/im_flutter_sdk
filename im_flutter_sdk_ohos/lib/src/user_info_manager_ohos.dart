...
class UserInfoManagerOhos extends UserInfoManager {
  ...
  Future<EMUserInfo> updateUserInfo({
    ...
    try {
      Map result = await UserInfoChannel.invokeMethod(
          ChatMethodKeys.updateOwnUserInfo, req);
  ...
  Future<EMUserInfo?> fetchOwnInfo({int expireTime = 0}) async {
    String? currentUser = await Client.instance.getCurrentUserId();
    if (currentUser == null) {
  ...
  Future<Map<String, EMUserInfo>> fetchUserInfoById(
    ...
    Map result = await UserInfoChannel.invokeMethod(
        ChatMethodKeys.fetchUserInfoById, req);
