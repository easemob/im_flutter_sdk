...
14. ✅ PR #629 三个 CI job 全红已修复：根因是 `example/lib/env.dart` 被 gitignore（内含集群凭据，不能提交），而 CI 没有任何步骤生成占位，全新检出下 example 编译不过——quality job 的 `flutter analyze` 报 6 条 `Target of URI doesn't exist: '../env.dart'` / `Undefined name 'environment'`，Android/iOS 编译 job 报 `Error when reading 'lib/env.dart': No such file or directory`。缺口自 09-18 起就存在（当时被 `dart format` 失败挡住而未暴露）。新增 `tool/ci/ensure_example_env.sh`（只从模板补 `example/lib/env.dart` 空占位、mode 0600、已存在则不覆盖；**刻意不生成 `example/config.local.json`**），由 `run_quality.sh`、两个 `run_*_test.sh` 与 `ci.yml` 的两个编译 job 调用；在「全新检出等价目录」中复刻三个 job 全部 exit 0（5 包 analyze + 34 测试 + Android/iOS 两个编译产物），并实测「删掉 env.dart → 复现 CI 的 6 条 error → 执行该步骤 → No issues found!」。详见 `04-verification.md` 第 13 节。
15. ✅ 项目侧移除多集群支持（用户裁决：产品支持多种集群，但实现侧不做「多选」，切换集群＝直接填值，便利性由使用者在 Git 外自行维护）：`config.local.json` 现在只描述当前要跑的那一个环境，`restApi` / `appKey` / `clientId` / `clientSecret` 提到顶层；移除了 `make env-use`、`.env/env.<cluster>.dart` 缓存与激活、`clusters` / `defaultCluster` / 每条资源的 `cluster` 字段、`env.dart` 里的 `cluster` 键与 auto 报告里的 `cluster` 元数据，并同步了 `CONTRIBUTING.md`、`AGENTS.md`、`example/README.md` 与 CI 文档。**保留私有化部署字段**（`enablePrivateConfig` + server 字段，它描述「这一个环境是不是私有化」，与选集群无关）。旧格式配置会得到一句明确的迁移报错（退出码 1）；本地 `config.local.json` 已拍平（备份在仓库外）并重跑 `make env-gettoken` 成功（ngi，2 个账号）。详见 `04-verification.md` 第 14 节。
