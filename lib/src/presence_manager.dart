...
class PresenceManager {
  ...
  static const MethodChannel _channel = const MethodChannel(
      '$_channelPrefix/presence_manager', JSONMethodCodec());

  final Map<String, PresenceEventHandler> _eventHandlesMap = {};
  ...
  ///
  /// Param [handler] The handle for presence event. See [PresenceEventHandler].
  /// ~end
  ///
  /// ~chinese
  /// 添加用户状态变化监听器。
  ...
  /// **Return** Presence 事件。
  /// ~end
  PresenceEventHandler? getEventHandler(String identifier) {
  ...
  ///
  /// **Throws** 如果有方法调用的异常会在这里抛出，可以看到具体错误原因。参见 [Error]。
  /// ~end
  ...
  Future<List<Presence>> subscribe({
    ...
    try {
      ...
      result[ChatMethodKeys.presenceSubscribe]?.forEach((element) {
        list.add(Presence.fromJson(element));
      });
      return list;
    } on Error catch (e) {
      throw e;
  ...
  Future<void> _presenceChange(Map event) async {
    ...
    for (var item in mapList) {
      pList.add(Presence.fromJson(item));
    }
