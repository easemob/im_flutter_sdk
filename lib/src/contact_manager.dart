...
class ContactManager {
  ...
  static const MethodChannel _channel =
      const MethodChannel('$_channelPrefix/contact_manager', JSONMethodCodec());
  ...
  ///
  /// Param [handler] 事件的句柄. See [ContactEventHandler].
  /// ~end
  ...
  /// **Throws** 如果有方法调用的异常会在这里抛出，可以看到具体错误原因。请参见 [Error]。
  /// ~end
  Future<Contact?> getContact({required String userId}) async {
    ...
    try {
      ...
      if (result.containsKey(ChatMethodKeys.getContact)) {
        return Contact.fromJson(result[ChatMethodKeys.getContact]);
      } else {
        return null;
      }
    } on Error catch (e) {
  ...
  /// **Throws** 如果有方法调用的异常会在这里抛出，可以看到具体错误原因。请参见 [Error]。
  /// ~end
  Future<CursorResult<Contact>> fetchContacts({
    String? cursor,
    int pageSize = 20,
  }) async {
    ...
    try {
      Error.hasErrorFromResult(result);
      return CursorResult.fromJson(result[ChatMethodKeys.fetchContacts],
