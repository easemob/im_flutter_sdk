...
class EMChatRoomManager {
  ...
  static const MethodChannel _emChatRoomManagerChannel = const MethodChannel(
  ...
  void joinChatRoom(String roomId,
    ...
    Future<Map> result = _emChatRoomManagerChannel
  ...
  void fetchPublicChatRoomsFromServer(int pageNum, int pageSize,
    ...
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) {
          ...
          emPageResult
              .getData()
              .forEach((page) => data.add(EMChatRoom.from(page)));
  ...
  void fetchChatRoomFromServer(String roomId,
      ...
      onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel
        .invokeMethod(EMSDKMethod.fetchChatRoomFromServer, {"roomId": roomId});
    result.then((response) {
      if (response["success"]) {
  ...
  Future<List> getAllChatRooms() async {
    ...
    if (result['success']) {
      ...
      List res = new List();
      list.forEach((data) => res.add(EMChatRoom.from(data)));
      return res;
    } else {
      return null;
    }
  ...
  void createChatRoom(String subject, String description, String welcomeMessage,
      int maxUserCount, List members,
      {onSuccess(Map emChatRoom), onError(int code, String desc)}) {
    Future<Map> result =
        _emChatRoomManagerChannel.invokeMethod(EMSDKMethod.createChatRoom, {
      "subject": subject,
  ...
  void destroyChatRoom(String roomId,
      {onSuccess(), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel
        .invokeMethod(EMSDKMethod.destroyChatRoom, {"roomId": roomId});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) onSuccess();
      } else {
  ...
  void changeChatRoomSubject(String roomId, String newSubject,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.changeChatRoomSubject,
        {"roomId": roomId, "newSubject": newSubject});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) onSuccess(response['value']);
      } else {
  ...
  /// @nodoc 如果修改成功，请调用[onSuccess]，如果出现错误，请调用[onError]。
  void changeChatRoomDescription(String roomId, String newDescription,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.changeChatRoomDescription,
        {"roomId": roomId, "newDescription": newDescription});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) onSuccess(response['value']);
      } else {
  ...
  /// 如果获取成功，请调用[onSuccess]，如果出现错误，请调用[onError]。
  void fetchChatRoomMembers(String roomId, String cursor, int pageSize,
      {onSuccess(EMCursorResult<String> result),
      onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.fetchChatRoomMembers,
        {"roomId": roomId, "cursor": cursor, "pageSize": pageSize});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) {
          List<String> list = [];
  ...
  void muteChatRoomMembers(String roomId, List muteMembers, String duration,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.muteChatRoomMembers,
        {"roomId": roomId, "muteMembers": muteMembers, "duration": duration});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) onSuccess(response['value']);
      } else {
  ...
  void unMuteChatRoomMembers(String roomId, List<String> muteMembers,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.unMuteChatRoomMembers,
        {"roomId": roomId, "muteMembers": muteMembers});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) onSuccess(response['value']);
      } else {
  ...
  void changeOwner(String roomId, String newOwner,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.changeChatRoomOwner,
        {"roomId": roomId, "newOwner": newOwner});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) onSuccess(response['value']);
      } else {
  ...
  /// 如果添加成功，请调用[onSuccess]，如果出现错误，请调用[onError]。
  void addChatRoomAdmin(String roomId, String admin,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
  ...
  void removeChatRoomAdmin(String roomId, String admin,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
  ...
  void fetchChatRoomMuteList(String roomId, int pageNum, int pageSize,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.fetchChatRoomMuteList,
  ...
  void removeChatRoomMembers(String roomId, List members,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.removeChatRoomMembers,
  ...
  void blockChatRoomMembers(String roomId, List members,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.blockChatRoomMembers,
        {"roomId": roomId, "members": members});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) onSuccess(response['value']);
  ...
  void unBlockChatRoomMembers(String roomId, List members,
      {onSuccess(Map map), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.unBlockChatRoomMembers,
        {"roomId": roomId, "members": members});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) onSuccess(response['value']);
  ...
  /// @nodoc 如果获取成功，请调用[onSuccess]，如果出现错误，请调用[onError]。
  void fetchChatRoomBlackList(String roomId, int pageNum, int pageSize,
      {onSuccess(List list), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.fetchChatRoomBlackList,
        {"roomId": roomId, "pageNum": pageNum, "pageSize": pageSize});
    result.then((response) {
      if (response["success"]) {
        if (onSuccess != null) onSuccess(response['value']);
  ...
  void updateChatRoomAnnouncement(String roomId, String announcement,
      {onSuccess(), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
        EMSDKMethod.updateChatRoomAnnouncement,
  ...
  void fetchChatRoomAnnouncement(String roomId,
      {onSuccess(String res), onError(int code, String desc)}) {
    Future<Map> result = _emChatRoomManagerChannel.invokeMethod(
