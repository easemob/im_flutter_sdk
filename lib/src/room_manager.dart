...
class ChatRoomManager {
  ...
  static const MethodChannel _channel =
      const MethodChannel('$_channelPrefix/room_manager', JSONMethodCodec());
  ...
  ///
  /// **Return** Chat room data. See [PageResult].
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  ...
  Future<PageResult<ChatRoom>> fetchPublicChatRoomsFromServer({
    ...
    try {
      Error.hasErrorFromResult(result);
      return PageResult<ChatRoom>.fromJson(
  ...
  ///
  /// **Return** The list of chat room members. See [CursorResult]. If [CursorResult.cursor] is an empty string (""), all data is fetched.
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  ...
  /// 获取聊天室成员列表。
  ///
  /// 返回的结果中，当 CursorResult.cursor 为空字符串 ("") 时，表示没有更多数据。
  ///
  /// Param [roomId] 聊天室 ID。
  ///
