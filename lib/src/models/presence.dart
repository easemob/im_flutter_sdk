...
class Presence {
  ...
  factory Presence.fromJson(Map map) {
...
class PresenceStatusDetail {
  ...
  PresenceStatusDetail._private(
    this.device,
    this.status,
  );

  factory PresenceStatusDetail.fromJson(Map map) {
    String device = map["device"];
    int status = map["status"] ?? 0;
    return PresenceStatusDetail._private(device, status);
  }
}
