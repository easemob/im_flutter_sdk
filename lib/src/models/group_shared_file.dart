...
/// ~english
/// The GroupSharedFile class, which manages the chat group shared files.
///
/// To get the information of the chat group shared file, call [GroupManager.fetchGroupFileListFromServer].
///
/// ```dart
///   List<GroupSharedFile>? list = await Client.getInstance.groupManager.fetchGroupFileListFromServer(groupId);
/// ```
/// ~end
///
/// ~chinese
/// 群组共享文件类。
///
/// 可以通过 [GroupManager.fetchGroupFileListFromServer] 方法获取共享文件信息，示例如下：
///
/// ```dart
///   List<GroupSharedFile> list = await Client.getInstance.groupManager.fetchGroupFileListFromServer(groupId);
/// ```
...
class GroupSharedFile {
  ...
  factory GroupSharedFile.fromJson(Map? map) {
    return GroupSharedFile._private()
      .._fileId = map?["fileId"]
      .._fileName = map?["name"]
