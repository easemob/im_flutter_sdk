...
/// ~english
/// The Options class, which contains the settings of the Chat SDK.
///
...
class Options {
  ...
  /// ~english
  /// Whether to output the debug information. Make sure to call the method after initializing the Client using [Client.init].
  ///
  /// - `true`: Yes.
  /// - (Default) `false`: No.
  /// ~end
  ///
  /// ~chinese
  /// 是否输出调试信息，在 Client 初始化完成后调用，详见 [Client.init]。
  ...
  /// ~end
  /// ~chinese
  /// 发送的消息是会执行 [ChatEventHandler.onMessagesReceived] 回调。
  ...
  /// This attribute can be set only when you call [Client.init]. The attribute setting cannot be changed during the app runtime.
  /// ~end
  ///
  /// ~chinese
  /// 区号
  /// 此属性用于限制可访问边缘节点的范围。缺省值为AreaCodeGLOB。
  /// 此属性只能在调用[Client.init]时设置。在应用程序运行期间不能更改属性设置。
  /// ~end
  ...
  PushConfig _pushConfig = PushConfig();

  @Deprecated('Use [PushManager.bindDeviceToken] instead.')
  ...
  ///
  /// Param [debugMode] Whether to output the debug information. Make sure to call the method after the Client is initialized.
  /// - `true`: Yes.
  /// - (Default) `false`: No.
  ...
  /// - `false`：否。
  ///
  /// Param [debugMode] 是否输出调试信息，在 Client 初始化完成后调用，详见 [Client.init]。
  /// - `true`：SDK 会在 log 里输出调试信息；
  /// - （默认）`false`：不会输出调试信息。
  ///
  ...
  @Deprecated('Use [Options.withAppKey] instead.')
