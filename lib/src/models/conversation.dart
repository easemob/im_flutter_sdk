...
///   Conversation? con = await Client.getInstance.chatManager.getConversation(conversationId);
...
class Conversation {
  ...
  factory Conversation.fromJson(Map<String, dynamic> map) {
    ...
    Conversation ret = Conversation._private(
      map["convId"],
  ...
  Future<Message?> latestMessage() async {
    ...
    try {
      ...
      if (result.containsKey(ChatMethodKeys.getLatestMessage)) {
        return Message.fromJson(result[ChatMethodKeys.getLatestMessage]);
      } else {
        return null;
      }
    } on Error catch (e) {
  ...
  /// **Return** 消息体实例。
  /// ~end
  Future<Message?> lastReceivedMessage() async {
    Map req = this._toJson();
    Map result = await _emConversationChannel.invokeMethod(
  ...
  /// **Return** 会话的消息未读数量。
  ///
  /// **Throws**  如果有异常会在此抛出，包含错误码和原因，详见 [Error]。
  /// ~end
  ...
  Future<Message?> loadMessage(String messageId) async {
    ...
    try {
      ...
      if (result[ChatMethodKeys.loadMsgWithId] != null) {
        return Message.fromJson(result[ChatMethodKeys.loadMsgWithId]);
      } else {
        return null;
      }
    } on Error catch (e) {
  ...
  /// - `SearchDirection.Up`: Messages are retrieved in the reverse chronological order of when the server received messages.
  /// - `SearchDirection.Down`: Messages are retrieved in the chronological order of when the server received messages.
  ///
  ...
  /// **Throws**  如果有异常会在这里抛出，包含错误码和错误描述，详见 [Error]。
  /// ~end
  Future<List<Message>> loadMessagesWithMsgType({
  ...
  Future<List<Message>> loadMessages({
    ...
    try {
      ...
      result[ChatMethodKeys.loadMsgWithStartId]?.forEach((element) {
        msgList.add(Message.fromJson(element));
      });
      return msgList;
    } on Error catch (e) {
      throw e;
  ...
  /// **Throws**  如果有异常会在这里抛出，包含错误码和错误描述，详见 [Error]。
  /// ~end
  Future<List<Message>> loadMessagesFromTime({
  ...
  /// **Throws** 如果有异常会在这里抛出，包含错误码和错误描述，详见 [Error]。
  /// ~end
  Future<List<Message>> loadPinnedMessages() async {
    Map req = this._toJson();
    Map result = await _emConversationChannel.invokeMethod(
  ...
  /// ~english
  /// The conversation no disturb type. [PushRemindType]
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  /// ~chinese
  /// 会话免打扰类型。[PushRemindType]。
  ///
  /// **Throws** 如果有异常会在这里抛出，包含错误码和错误描述，详见 [Error]。
  /// ~end
  ///
  Future<PushRemindType> remindType() async {
    Map req = this._toJson();
    Map result = await _emConversationChannel.invokeMethod(
        ChatMethodKeys.conversationRemindType, req);
    try {
      Error.hasErrorFromResult(result);
      return PushRemindType
