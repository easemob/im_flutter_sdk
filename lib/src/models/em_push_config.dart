...
extension EMPushConfigExtension on EMImPushConfig {
  ...
  /// 通过群id[groupId]设置群组是否接收推送[enablePush], 默认群组推送都是开启的，如果需要关闭某个群组的推送，需要传NO。
  Future<EMGroup> setGroupEnablePush(
    String groupId,
    bool enablePush,
  ) async {
    Map req = {'enablePush': enablePush, 'group_id': groupId};
    ...
    if (enablePush) _noDisturbGroups.add(group.groupId);
    return group;
  }
  ...
  Future<List<String>> noDisturbGroupsFromServer() async {
    ...
    EMError.hasErrorFromResult(result);
    _noDisturbGroups =
        result[EMSDKMethod.getNoDisturbGroups]?.cast<String>() ?? [];
