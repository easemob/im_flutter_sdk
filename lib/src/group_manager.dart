...
class GroupManager {
  ...
  static const MethodChannel _channel =
      const MethodChannel('$_channelPrefix/group_manager', JSONMethodCodec());

  final Map<String, GroupEventHandler> _eventHandlesMap = {};
  ...
  ///
  /// Param [handler] The handle for group event. See [GroupEventHandler].
  /// ~end
  ///
  /// ~chinese
  /// 添加群组事件监听。
  ...
  ///
  /// Param [handler] 群组事件监听，请见 [GroupEventHandler].
  /// ~end
  ...
  /// **Return** ID 对应的监听。
  /// ~end
  GroupEventHandler? getEventHandler(String identifier) {
  ...
  /// ~end
  Future<Group?> getGroupWithId(String groupId) async {
    ...
    try {
      ...
      if (result.containsKey(ChatMethodKeys.getGroupWithId)) {
        return Group.fromJson(result[ChatMethodKeys.getGroupWithId]);
      } else {
        return null;
      }
    } on Error catch (e) {
  ...
  Future<List<Group>> getJoinedGroups() async {
    ...
    try {
      ...
      result[ChatMethodKeys.getJoinedGroups]
          ?.forEach((element) => list.add(Group.fromJson(element)));
      return list;
    } on Error catch (e) {
      throw e;
    }
  ...
  /// 从服务器中获取当前用户加入的所有群组。
  ///
  /// 此操作只返回群组列表，不包含所有成员的信息。如果要更新某个群组包括成员的全部信息，需要再调用 [GroupManager.fetchGroupInfoFromServer]。
  ///
  /// **Return** 当前用户加入的群组的列表。
  ///
  /// **Throws**  如果有异常会在此抛出，包括错误码和错误信息，详见 [Error]。
  /// ~end
  Future<List<Group>> fetchJoinedGroupsFromServer({
    int pageSize = 20,
    int pageNum = 0,
  ...
  ///
  /// **Return** The result of [CursorResult], including the cursor for getting data next time and the group list.
  /// If [CursorResult.cursor] is an empty string (""), all data is fetched.
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  ...
  ///
  /// **Return** 包含用于下次获取数据的 cursor 以及群组列表。返回的结果中，当 `CursorResult.getCursor()` 为空字符串 ("") 时，表示没有更多数据。
  ///
  /// **Throws**  如果有异常会在此抛出，包括错误码和错误信息，详见 [Error]。
  /// ~end
  Future<CursorResult<GroupInfo>> fetchPublicGroupsFromServer({
    int pageSize = 200,
    String? cursor,
  }) async {
    ...
    try {
      Error.hasErrorFromResult(result);
      return CursorResult<GroupInfo>.fromJson(
          result[ChatMethodKeys.getPublicGroupsFromServer],
          dataItemCallback: (value) {
        return GroupInfo.fromJson(value);
      });
    } on Error catch (e) {
      throw e;
    }
  ...
  /// You can set [MultiDeviceEventHandler] to listen for the event. If an event occurs, the callback function
  /// [MultiDeviceEventHandler.onGroupEvent] is triggered, where the first parameter is the event which is
  /// [MultiDevicesEvent.GROUP_CREATE] for a group creation event.
  ///
  ...
  ///
  /// Param [options] The options for creating a group. See [GroupOptions].
  /// The options are as follows:
  /// - The maximum number of group members. The default value is 200.
  /// - The group style. See [GroupStyle]. The default value is [GroupStyle.PrivateOnlyOwnerInvite].
  ...
  /// 可通过设置 [Client.addMultiDeviceEventHandler] 监听相关事件，事件回调函数为 [MultiDeviceEventHandler.onGroupEvent]，第一个参数为事件，创建群组事件为 [MultiDevicesEvent.GROUP_CREATE]。
  ///
  ...
  ///
  /// Param [options] 群组的其他选项。请参见 [GroupOptions]。
  /// 群组的其他选项。
  /// - 群最大成员数，默认值为 200；
  /// - 群组类型，详见 [GroupStyle]，默认为 [GroupStyle.PrivateOnlyOwnerInvite]；
  ...
  Future<Group> createGroup({
    ...
    try {
      Error.hasErrorFromResult(result);
      return Group.fromJson(result[ChatMethodKeys.createGroup]);
    } on Error catch (e) {
      throw e;
    }
  }
  ...
  /// **Throws**  如果有异常会在此抛出，包括错误码和错误信息，详见 [Error]。
  /// ~end
  Future<Group> fetchGroupInfoFromServer(
    String groupId, {
  ...
  /// For example:
  ///   ```dart
  ///     CursorResult<String> result = await Client.getInstance.groupManager.fetchMemberListFromServer(groupId); // search 1
  ///     result = await Client.getInstance.groupManager.fetchMemberListFromServer(groupId, cursor: result.cursor); // search 2
  ///   ```
  ///
  /// Param [groupId] The group ID.
  ///
  ...
  /// Param [cursor] The cursor position from which to start to get data next time. Sets the parameter as null for the first time.
  ///
  /// **Return** The result of [CursorResult], including the cursor for getting data next time and the group member list.
  /// If [CursorResult.cursor] is an empty string (""), all data is fetched.
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ...
  ///
  /// **Return** 分页获取结果 [CursorResult]，包含用于下次获取数据的 cursor 以及群组成员列表。返回的结果中，当 [CursorResult.cursor] 为空字符串 ("") 时，表示没有更多数据。
  ///
  /// **Throws**  如果有异常会在此抛出，包括错误码和错误信息，详见 [Error]。
  /// ~end
  Future<CursorResult<String>> fetchMemberListFromServer(
    String groupId, {
    int pageSize = 200,
    String? cursor,
  }) async {
    ...
    try {
      Error.hasErrorFromResult(result);
      return CursorResult<String>.fromJson(
  ...
  /// **Throws**  如果有异常会在此抛出，包括错误码和错误信息，详见 [Error]。
  /// ~end
  Future<List<GroupSharedFile>> fetchGroupFileListFromServer(
    String groupId, {
    int pageSize = 200,
    int pageNum = 1,
  }) async {
    ...
    try {
      Error.hasErrorFromResult(result);
      List<GroupSharedFile> list = [];
      result[ChatMethodKeys.getGroupFileListFromServer]?.forEach((element) {
        list.add(GroupSharedFile.fromJson(element));
      });
      return list;
    } on Error catch (e) {
      throw e;
  ...
  ///
  /// This method works only for groups with the style of [GroupStyle.PrivateOnlyOwnerInvite], [GroupStyle.PrivateMemberCanInvite], or [GroupStyle.PublicJoinNeedApproval].
  /// For a group with the [GroupStyle.PrivateOnlyOwnerInvite] style, only the group owner can invite users to join the group;
  /// For a group with the [GroupStyle.PrivateMemberCanInvite] style, each group member can invite users to join the group.
  ///
  /// Param [groupId] The group ID.
  ///
  ...
  /// 邀请用户加入群组。
  ///
  /// 群类型为 [GroupStyle.PrivateOnlyOwnerInvite]、[GroupStyle.PrivateMemberCanInvite] 和 [GroupStyle.PublicJoinNeedApproval] 的群组可以邀请用户加入。
  ///
  /// - 对于 [GroupStyle.PrivateOnlyOwnerInvite] 属性的群组，仅群主可邀请用户入群。
  /// - 对于 [GroupStyle.PrivateMemberCanInvite] 属性的群组，群成员可邀请用户入群。
  /// - 对于 [GroupStyle.PublicJoinNeedApproval] 属性的群组，仅群主可邀请用户加入。
  ///
  /// Param [groupId] 群组 ID。
  ///
  ...
  /// Param [name] 修改后的群组名称。
  ///
  /// **Throws**  如果有异常会在此抛出，包括错误码和错误信息，详见 详见 [Error]。
  /// ~end
  ...
  /// For a group that requires authentication, users need to wait for the group owner to agree before joining the group. For details, See [GroupStyle].
  ///
  /// Param [groupId] The group ID.
  ///
  /// **Throws** A description of the exception. See [Error].
  ...
  /// 当前登录用户加入公开群。
  ///
  /// 若是自由加入的公开群[GroupStyle.PublicOpenJoin] ，直接进入群组；若公开群需验证，群主同意后才能入群，使用 [requestToJoinPublicGroup] 方法。
  ///
  /// Param [groupId] 群组 ID。
  ///
  /// **Throws**  如果有异常会在此抛出，包括错误码和错误信息，详见 [Error]。
  ...
  ///
  /// This method works only for public groups requiring authentication, i.e., groups with the style of [GroupStyle.PublicJoinNeedApproval].
  ///
  /// Param [groupId] The group ID.
  ///
  ...
  /// 申请加入群组。
  ///
  /// 该方法仅适用于需要验证的公开群组，即类型为 [GroupStyle.PublicJoinNeedApproval] 的群组。
  ///
  /// Param [groupId] 群组 ID。
  ///
  ...
  /// Param [groupId] The group ID.
  ///
  /// Param [userId] The username of the user who sends a request to join the group.
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  ...
  Future<void> acceptJoinApplication({
    ...
    Map req = {'groupId': groupId, 'username': userId};
    Map result =
  ...
  Future<Group> acceptInvitation({
    ...
    try {
      Error.hasErrorFromResult(result);
      return Group.fromJson(result[ChatMethodKeys.acceptInvitationFromGroup]);
    } on Error catch (e) {
      throw e;
    }
  }
  ...
  Future<void> _onGroupChanged(Map? map) async {
    ...
    _eventHandlesMap.values.forEach((element) {
      switch (type) {
        case GroupChangeEvent.ON_INVITATION_RECEIVED:
        ...
        case GroupChangeEvent.ON_AUTO_ACCEPT_INVITATION:
          ...
          break;
        case GroupChangeEvent.ON_USER_REMOVED:
          String groupId = map['groupId'];
          String? groupName = map['groupName'];
          element.onUserRemovedFromGroup?.call(groupId, groupName);
          break;
        case GroupChangeEvent.ON_REQUEST_TO_JOIN_RECEIVED:
          String groupId = map['groupId'];
          String? groupName = map['groupName'];
          ...
          break;
        case GroupChangeEvent.ON_REQUEST_TO_JOIN_DECLINED:
          String groupId = map['groupId'];
          String? groupName = map['groupName'];
          String? applicant = map['applicant'];
          String? reason = map['reason'];
          ...
          break;
        case GroupChangeEvent.ON_REQUEST_TO_JOIN_ACCEPTED:
          String groupId = map['groupId'];
          String? groupName = map['groupName'];
          ...
          break;
        case GroupChangeEvent.ON_GROUP_DESTROYED:
          String groupId = map['groupId'];
          String? groupName = map['groupName'];
          element.onGroupDestroyed?.call(groupId, groupName);
          break;
        case GroupChangeEvent.ON_MUTE_LIST_ADDED:
          String groupId = map['groupId'];
