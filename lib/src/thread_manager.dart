...
class ThreadManager {
  ...
  static const MethodChannel _channel =
      const MethodChannel('$_channelPrefix/thread_manager', JSONMethodCodec());

  final Map<String, ThreadEventHandler> _eventHandlesMap = {};
  ...
  ///
  /// Param [handler] The handle for chat thread event. See [ThreadEventHandler].
  /// ~end
  ///
  /// ~chinese
  /// 添加 Thread 事件监听。
  ...
  ///
  /// Param [handler] 事件监听. 请见 [ThreadEventHandler].
  /// ~end
  ...
  /// **Return** Thread 事件监听。
  /// ~end
  ThreadEventHandler? getEventHandler(String identifier) {
  ...
  ///
  /// **Return** Returns the result of [CursorResult], including the cursor for getting data next time and the chat thread object list.
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  ...
  /// Param [limit] The number of fetches at one time. Value range [1, 50].
  ///
  /// **Return** result of [CursorResult], including the cursor for getting data next time and the chat thread object list.
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  ...
  /// Param [limit] The number of fetches at one time. Value range [1, 50].
  ///
  /// **Return** The result of [CursorResult], including the cursor for getting data next time and the chat thread object list.
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  ...
  /// Param [limit] The number of fetches at one time. Value range [1, 50].
  ///
  /// **Return** The result of [CursorResult], including the cursor for getting data next time and the chat thread member list.
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  ...
  ///
  /// **Return** 若调用成功，返回子区成员 [CursorResult]；失败则抛出异常。
  ///
  /// **Throws** 如果有异常会在此抛出，包括错误码和错误信息，详见 [Error]。
  /// ~end
  ...
  Future<Map<String, Message>> fetchLatestMessageWithChatThreads({
    ...
    try {
      ...
      for (var key in map.keys) {
        ...
        ret[key] = Message.fromJson(msgMap);
      }
      return ret;
    } on Error catch (e) {
      throw e;
  ...
  ///
  /// 被移出的成员会收到 [ThreadEventHandler.onUserKickOutOfChatThread] 回调。
  ///
  ...
  /// You can set [ThreadEventHandler.onChatThreadUpdate] to listen on the event.
  ///
  /// Param [chatThreadId] Chat Thread ID.
  ///
  ...
  /// 只有子区所属群主、群管理员及子区创建者可调用该方法。
  ///
  /// 子区所属群组的所有成员均会收到 [ThreadEventHandler.onChatThreadUpdate]。
  ///
  /// Param [chatThreadId] 子区 ID。
  ///
  ...
  /// and can listen to related events by setting [ThreadEventHandler].
  /// The event callback function is [ThreadEventHandler.onChatThreadCreate].
  /// 2. Multiple devices will receive the notification event and you can set [MultiDeviceEventHandler] to listen on the event.
  /// The event callback function is [MultiDeviceEventHandler.onChatThreadEvent], where the first parameter is the event,
  /// for example, [MultiDevicesEvent.CHAT_THREAD_CREATE] for the chat thread creation event.
  ///
  ...
  /// 子区创建成功后，会出现如下情况：
  ///
  /// - 单设备登录时，子区所属群组的所有成员均会收到 [ThreadEventHandler.onChatThreadCreate] 。
  ///
  /// - 多端多设备登录时，各设备会收到 [MultiDeviceEventHandler.onChatThreadEvent] 回调。
  ///
  ...
  /// Repeated addition will throw an Error.
  /// After joining chat thread, the multiple devices will receive the notification event.
  /// You can set [MultiDeviceEventHandler] to listen on the event.
  /// The event callback function is [MultiDeviceEventHandler.onChatThreadEvent],
  /// where the first parameter is the event, and chat thread join event is [MultiDevicesEvent.CHAT_THREAD_JOIN].
  ///
  /// Param [chatThreadId] Chat Thread ID.
  ///
  ...
  /// 子区所属群组的所有成员均可调用该方法。
  ///
  /// 加入成功后，多端多设备登录情况下，其他设备会收到 [MultiDeviceEventHandler.onChatThreadEvent]，Event 的值为 [MultiDevicesEvent.CHAT_THREAD_JOIN]。
  ///
  /// Param [chatThreadId] 子区 ID。
  ///
  ...
  /// You can set {@MultiDeviceEventHandler} to listen on the event.
  /// The event callback function is [MultiDeviceEventHandler.onChatThreadEvent],
  /// where the first parameter is the event, and chat thread exit event is [MultiDevicesEvent.CHAT_THREAD_LEAVE].
  ///
  /// Param [chatThreadId] Chat Thread ID.
  ///
  /// **Throws** A description of the exception. See [Error].
  ...
  /// @note
  /// 子区中的所有成员均可调用该方法。
  /// 多端多设备登录情况下，其他设备会收到 [MultiDeviceEventHandler.onChatThreadEvent] 回调，Event 的值为 [MultiDevicesEvent.CHAT_THREAD_LEAVE]。
  ///
  ...
  /// and can listen to related events by setting [ThreadEventHandler].
  /// The event callback function is [ThreadEventHandler.onChatThreadDestroy].
  /// 2. Multiple devices will receive the notification event and you can set [MultiDeviceEventHandler] to listen on the event.
  /// The event callback function is [MultiDeviceEventHandler.onChatThreadEvent], where the first parameter is the event,
  /// for example, [MultiDevicesEvent.CHAT_THREAD_DESTROY] for the chat thread destruction event.
  ///
  /// Param [chatThreadId] Chat Thread ID.
  ///
  /// **Throws** A description of the exception. See [Error].
  ...
  /// 单设备登录时，子区所在群的所有成员均会收到 [ThreadEventHandler.onChatThreadDestroy] 。
  /// 多端多设备登录时，其他设备会收到 [MultiDeviceEventHandler.onChatThreadEvent] 回调，Event 的值为 [MultiDevicesEvent.CHAT_THREAD_DESTROY]。
  ///
  /// Param [chatThreadId] 子区 ID。
  ///
  /// **Throws** 如果有异常会在此抛出，包括错误码和错误信息，详见 [Error]。
