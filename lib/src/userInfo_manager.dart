...
class UserInfoManager {
  ...
  static const MethodChannel _channel = const MethodChannel(
      '$_channelPrefix/userInfo_manager', JSONMethodCodec());

  // The map of effective contacts.
  Map<String, UserInfo> _effectiveUserInfoMap = Map();
  ...
  /// 修改当前用户的属性信息。
  ///
  /// Param [nickname] 用户昵称。该昵称与推送设置中的昵称设置不同，我们建议这两种昵称的设置保持一致。设置推送昵称详见 [PushManager.updatePushNickname]。
  ///
  ...
  Future<UserInfo> updateUserInfo({
    ...
    try {
      ...
      UserInfo info =
          UserInfo.fromJson(result[ChatMethodKeys.updateOwnUserInfo]);
  ...
  ///
  /// **Return** The user properties that are obtained. See [UserInfo].
  ///
  /// **Throws** A description of the exception. See [Error].
  /// ~end
  ///
  ...
  ///
  /// **Return** 用户属性。请参见 [UserInfo]。
  ///
  /// **Throws**  如果有方法调用的异常会在这里抛出，可以看到具体错误原因。请参见 [Error]。
  /// ~end
  Future<UserInfo?> fetchOwnInfo({int expireTime = 0}) async {
    String? currentUser = await Client.getInstance.getCurrentUserId();
    if (currentUser != null) {
      try {
        Map<String, UserInfo> ret = await fetchUserInfoById(
          [currentUser],
  ...
  /// **Throws** 如果有方法调用的异常会在这里抛出，可以看到具体错误原因。请参见 [Error]。
  /// ~end
  Future<Map<String, UserInfo>> fetchUserInfoById(
    List<String> userIds, {
    int expireTime = 0,
  }) async {
        ...
        .where((element) =>
            ...
            (_effectiveUserInfoMap.containsKey(element) &&
                DateTime.now().millisecondsSinceEpoch -
                    ...
                    expireTime * 1000))
        .toList();
    Map<String, UserInfo> resultMap = Map();
    ...
    try {
      ...
      result[ChatMethodKeys.fetchUserInfoById]?.forEach((key, value) {
        UserInfo eUserInfo = UserInfo.fromJson(value);
